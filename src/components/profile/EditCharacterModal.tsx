...
import { X, Loader2, Check, Save, Trash2 } from 'lucide-react';
...
export default function EditCharacterModal({
    ...
    // Form state - initialized from character prop
    const [name, setName] = useState(character.name);
    const [wowClass, setWowClass] = useState<WowClass>(character.wowClass);
    const [race, setRace] = useState<WowRace>(character.race);
    const [role, setRole] = useState<Role>(character.role);
    const [rank, setRank] = useState<GuildRank>(character.rank);
    const [level, setLevel] = useState(character.level);
    const [professions, setProfessions] = useState<string[]>(character.professions || []);
    const [upgradesUrl, setUpgradesUrl] = useState(character.upgradesUrl || '');
    const [isMain, setIsMain] = useState(character.isMain);

    // Update form when character prop changes
    useEffect(() => {
        setName(character.name);
        setWowClass(character.wowClass);
        setRace(character.race);
        setRole(character.role);
        setRank(character.rank);
        setLevel(character.level);
        setProfessions(character.professions || []);
        setUpgradesUrl(character.upgradesUrl || '');
        setIsMain(character.isMain);
        setError(null);
    }, [character]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log('[EditCharacter] Updating character:', character.id);

            // If this character will become main, unset other mains first
            if (isMain && !character.isMain) {
                console.log('[EditCharacter] Setting as main, unsetting others...');
                await supabase
                    .from('characters')
            ...
            if (updateError) {
                console.error('[EditCharacter] Update error:', updateError);
            ...
            console.log('[EditCharacter] Character updated successfully');
            ...
            <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-obsidian-light border border-stone-border rounded-xl shadow-2xl">
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between p-6 border-b border-stone-border bg-obsidian-light">
                    <h2 className="gothic-heading text-xl text-text-primary flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-ethereal-purple/20 flex items-center justify-center">
                            <ClassIcon wowClass={character.wowClass} size="sm" />
                        </div>
                        Edit {character.name}
                    </h2>
                    <button
